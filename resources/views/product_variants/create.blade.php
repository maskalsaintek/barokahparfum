    ...
    <form action="{{ route('product-variants.store') }}" method="POST">
        ...
        <div class="field">
            ...
            <select name="fragrance_id" id="fragranceSelect" size="6" style="width:100%; margin-top:4px;">
                @foreach($fragrances as $fr)
                    <option value="{{ $fr->id }}"
                        {{ old('fragrance_id') == $fr->id ? 'selected' : '' }}>
                        {{ $fr->code }} - {{ $fr->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="field">
            ...
            <select name="variant_type_id" id="variantTypeSelect" size="4" style="width:100%; margin-top:4px;">
                @foreach($variantTypes as $vt)
                    <option value="{{ $vt->id }}"
                        {{ old('variant_type_id') == $vt->id ? 'selected' : '' }}>
                        {{ $vt->code }} - {{ $vt->name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="field">
            ...
            <input type="text" name="bottle_size_ml" id="bottle_size_ml"
                   value="{{ old('bottle_size_ml') }}" placeholder="contoh: 12">
        ...
        <div class="field">
            <label for="base_price">Base Price (Total Harga)</label>
            <input type="text" name="base_price" id="base_price"
                   value="{{ old('base_price') }}" placeholder="contoh: 36000">
            @error('base_price')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="field">
            <label for="cost_price">Cost Price / ML</label>
            <input type="text" name="cost_ml" id="cost_ml"
                   value="{{ old('cost_ml') }}" placeholder="contoh: 800">
            @error('cost_ml')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="field">
            <label for="mix_ratio">Mix Ratio (optional)</label>
            <input type="text" name="mix_ratio" id="mix_ratio"
                   value="{{ old('mix_ratio') }}" placeholder="contoh: 1:1, 2:1">
            @error('mix_ratio')
                <div class="error">{{ $message }}</div>
            @enderror
        </div>
        ...
        <a href="{{ route('product-variants.index') }}" class="btn btn-secondary">Cancel</a>
    </form>
